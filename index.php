...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        echo json_encode(['success'=>true,'id'=>$id,'name'=>$name,'email'=>$email,'phone'=>$phone,'age'=>$age,'class'=>$class]);
        exit;
    }
    ...
    if ($action === 'edit') {
        ...
        if(isset($_SESSION['students'][$id])){
            ...
            $_SESSION['students'][$id]['email'] = htmlspecialchars(trim($_POST['email'] ?? ''));
            $_SESSION['students'][$id]['phone'] = htmlspecialchars(trim($_POST['phone'] ?? ''));
...
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500&family=Outfit:wght@400;600;700&display=swap" rel="stylesheet">
</head>
<body>
<div class="wrapper">
    <header>
...
<div class="modal-overlay" id="record-modal">
    <div class="modal">
        ...
        <input type="text" id="student-phone" placeholder="Enter phone number">
...
function openAddModal(){
    ...
    ['edit-id','student-name','student-email','student-phone','student-age','student-class'].forEach(id=>document.getElementById(id).value='');
